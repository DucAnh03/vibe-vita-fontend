...
export default function TrainerProfile() {
  ...
  return (
    <div className="trainer-container">
      ...
      <div className="trainer-right">
        <h1 className="hello-text">
          XIN CHÀO, <span>{trainer.username?.toUpperCase()}!</span>
        </h1>
        ...
        <section className="info-section">
          <h3>THÔNG TIN CƠ BẢN</h3>
          {[
            { label: "Tên huấn luyện viên", name: "username" },
            { label: "Email", name: "email" },
            { label: "Giới tính", name: "gender" },
            { label: "Địa chỉ", name: "location" },
            { label: "Chuyên môn", name: "specialty" },
            { label: "Kinh nghiệm", name: "experience" },
          ].map((field) => (
            <div className="info-row" key={field.name}>
              <p>{field.label}</p>
              <input
                name={field.name}
