...
export default function ListTrainers() {
  ...
  const API_URL = process.env.REACT_APP_API_URL; // ✅ Lấy từ ENV (Vercel)

  useEffect(() => {
    const fetchTrainers = async () => {
      try {
        const res = await fetch(`${API_URL}/api/auth/trainers`);
    ...
    <div className="pt-page">
      ...
      <div className="pt-grid">
        {trainers.length === 0 ? (
          ...
          trainers.map((t) => (
            <article key={t._id} className="pt-card">
              <div className="pt-image-wrap">
                <img
                  src={
                    t.image ||
              ...
              <ul className="pt-info">
                <li>
                  <DumbbellIcon />
                  <span>{t.specialty || "Chưa cập nhật"}</span>
                </li>
                <li>
                  <StarIcon />
                  <span>
                ...
                <li>
                  <BadgeIcon />
                  <span>{t.experience || "Chưa có thông tin"}</span>
                </li>
                <li>
                  <PinIcon />
              ...
              <div className="pt-actions">
                <button className="pt-btn primary">ĐẶT LỊCH</button>
                <button className="pt-btn ghost">CHI TIẾT</button>
              </div>
            </article>
          ))
        )}
...
function BadgeIcon() {
  return (
    <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
      <path d="M12 2 3 7v7c0 4 6 8 9 8s9-4 9-8V7l-9-5Z" fill="currentColor" />
    </svg>
  );
}
function PinIcon() {
